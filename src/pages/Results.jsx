...
export default function Results({ results, athletes, onRefresh, onNav, profile }) {
  ...
  // safe list — guard against undefined results
  ...
  async function handleSave(formData) {
    ...
    if (!payload.athlete_name || !payload.event_name) {
      toast('Athlete and competition required', 'error'); return
    }
    const { error } = isEdit
  ...
  return (
    <div>
      ...
      {confirm && (
        <ConfirmModal
          title="Delete result"
          message="Delete this result? Medal counts will update."
      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">{list.length} of {safeResults.length} results</div>
        </div>
        {canEdit(profile) && (
          <button className="btn btn-purple" onClick={() => setForm('new')}>
      ...
      <div className="filters">
        ...
        <select className="filter" value={sport} onChange={e => setSport(e.target.value)}>
          {sports.map(s => <option key={s}>{s}</option>)}
        </select>
        <select className="filter" value={sort} onChange={e => setSort(e.target.value)}>
          <option value="date-desc">Date (newest)</option>
          <option value="date-asc">Date (oldest)</option>
          <option value="athlete-asc">Athlete A→Z</option>
      ...
      <div className="tbl-wrap" style={{ marginBottom: 16 }}>
        <table>
          <thead>
            <tr>
              <th>Medal</th><th>Athlete</th><th>Discipline</th>
              <th>Competition</th><th>Result</th><th>Date</th>
              {canEdit(profile) && <th />}
            </tr>
          </thead>
          <tbody>
                    ...
                    <td>
                      <div style={{ display:'flex', gap:6 }}>
                        <button
                          ...
                          onClick={e => {
                            e.stopPropagation()
                            setForm({ id:r.id, athleteName:r.athlete_name, medal:r.medal, eventName:r.event_name, discipline:r.discipline, result:r.result, position:r.position, date:r.date })
                          }}>
            ...
            {list.length === 0 && (
              <tr><td colSpan={canEdit(profile) ? 7 : 6}>
                <div className="empty">No results match</div>
              </td></tr>
            )}
          </tbody>
        </table>
      ...
      <div className="card">
        ...
        <div className="medal-tally-grid">
          ...
          {tallyAthletes.length === 0 && <div className="empty">No medals yet</div>}
        </div>
      </div>
    </div>
  )
