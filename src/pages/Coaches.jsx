  ...
  return (
    <div>
      ...
      <div className="filters">
        ...
        <select className="filter" value={status} onChange={e => setStatus(e.target.value)}>{[['All statuses',tx('filters.allStatuses','All statuses')],['Active',tx('status.active','Active')],['On Leave',lang==='ar'?'في إجازة':'On Leave'],['In Competition',lang==='ar'?'في منافسة':'In Competition'],['In Training Camp',lang==='ar'?'في معسكر تدريبي':'In Training Camp'],['Inactive',tx('status.inactive','Inactive')]].map(([val,lbl]) => <option key={val} value={val}>{lbl}</option>)}</select>
