...
// Country/nationality → Arabic translation map. Exported as a plain,
// non-hook function (`translateCountry`) so non-component code that can't
// call `useLang()` — e.g. the Athletes PDF/Excel export functions, which
// run outside React — can still use the exact same mapping the rest of
// the app uses via `tc()`, instead of maintaining a separate copy.
const COUNTRY_MAP = {
...
export function translateCountry(country, lang) {
  if (!country || lang !== 'ar') return country
...
export function LangProvider({ children }) {
  ...
  // Translate country name — thin wrapper around the shared translateCountry()
  const tc = (country) => translateCountry(country, lang)
