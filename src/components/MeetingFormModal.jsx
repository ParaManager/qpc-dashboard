...
export default function MeetingFormModal({ meeting, onClose, onSaved, onDelete, profile }) {
  ...
  const [form, setForm] = useState({
    ...
    attendeeIds: (meeting?.meeting_attendees || []).map(a => a.employee_id),
  })

  useEffect(() => {
    // Attendees are drawn from Employees only (any employee, regardless of
    // whether they also have a linked person record) — athletes, coach-only,
    // and referee records are excluded unless they also exist as Employees.
    supabase.from('employees')
      .select('id, name, name_ar, designation, designation_ar, status')
  ...
  async function handleSave() {
    ...
    if (form.attendeeIds.length) {
      const { error: insErr } = await supabase.from('meeting_attendees')
        .insert(form.attendeeIds.map(eid => ({ meeting_id: meetingId, employee_id: eid })))
