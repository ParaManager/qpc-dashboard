      ...
      <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
        <div className="modal-box" style={{ maxWidth:400 }} onClick={e => e.stopPropagation()}>
          ...
          <div className="modal-footer">
            <button className="btn-cancel" onClick={() => setConfirmDelete(null)} disabled={deleting}>{L('Cancel','إلغاء')}</button>
            <button className="btn" style={{ background:'#EE334E', opacity: deleting ? 0.7 : 1, cursor: deleting ? 'default' : 'pointer' }} onClick={() => deleteAccount(confirmDelete.id)} disabled={deleting}>
              {deleting
                ? <><i className="ti ti-loader-2" style={{ animation:'spin 0.6s linear infinite' }} /> {L('Deleting…','جارٍ الحذف...')}</>
