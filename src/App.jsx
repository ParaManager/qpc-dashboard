...
import SpecialOlympics from './pages/SpecialOlympics'
...
const NAV_ADMIN = (tx) => [
  ...
  { section: tx('nav.people','People'),          items: [{ id:'athletes', icon:'ti-run', label:tx('nav.athletes','Athletes') }, { id:'coaches', icon:'ti-user-star', label:tx('nav.coaches','Coaches') }, { id:'employees', icon:'ti-users', label:tx('nav.employees','Staff') }, { id:'referees', icon:'ti-award', label:tx('nav.referees','Referees') }] },
  ...
  { section: tx('nav.competitions','Competitions'), items: [{ id:'sports', icon:'ti-ball-football', label:tx('nav.sports','Sports') }, { id:'special-olympics', icon:'ti-medal-2', label:tx('nav.specialOlympics','Special Olympics') }, { id:'events', icon:'ti-calendar-event', label:tx('nav.events','Events') }, { id:'results', icon:'ti-medal', label:tx('nav.results','Results') }] },
...
const NAV_EMPLOYEE = (tx) => [
  { section: tx('nav.overview','Overview'),      items: [{ id:'dashboard', icon:'ti-layout-dashboard', label:tx('nav.dashboard','Dashboard') }, { id:'notifications', icon:'ti-bell', label:tx('nav.notifications','Notifications') }, { id:'calendar', icon:'ti-calendar', label:tx('nav.calendar','Calendar') }, { id:'resources', icon:'ti-folder', label:tx('nav.resources','Resources') }, { id:'requests', icon:'ti-clipboard-text', label:tx('nav.requests','Requests') }, { id:'tasks', icon:'ti-checklist', label:tx('nav.tasks','Tasks') }, { id:'profile', icon:'ti-user-circle', label:tx('nav.profile','My Profile') }] },
  { section: tx('nav.people','People'), items: [{ id:'athletes-all', icon:'ti-run', label:tx('nav.athletes','Athletes') }, { id:'coaches', icon:'ti-user-star', label:tx('nav.coaches','Coaches') }, { id:'employees', icon:'ti-id-badge-2', label:tx('nav.employees','Staff') }, { id:'referees', icon:'ti-flag-2', label:tx('nav.referees','Referees') }] },
  { section: tx('nav.competitions','Competitions'), items: [{ id:'sports', icon:'ti-ball-football', label:tx('nav.sports','Sports') }, { id:'special-olympics', icon:'ti-medal-2', label:tx('nav.specialOlympics','Special Olympics') }, { id:'events', icon:'ti-calendar-event', label:tx('nav.events','Events') }, { id:'results', icon:'ti-medal', label:tx('nav.results','Results') }] },
  { section: tx('nav.account','Account'),         items: [{ id:'settings', icon:'ti-settings', label:tx('nav.settings','Settings') }] },
]
const NAV_ATHLETE = (tx) => [
  { section: tx('nav.overview','Overview'),      items: [{ id:'athlete-dashboard', icon:'ti-layout-dashboard', label:tx('nav.dashboard','Dashboard') }, { id:'notifications', icon:'ti-bell', label:tx('nav.notifications','Notifications') }, { id:'calendar', icon:'ti-calendar', label:tx('nav.calendar','Calendar') }, { id:'resources', icon:'ti-folder', label:tx('nav.resources','Resources') }, { id:'requests', icon:'ti-clipboard-text', label:tx('nav.requests','Requests') }, { id:'profile', icon:'ti-user-circle', label:tx('nav.profile','My Profile') }] },
  { section: tx('nav.training','Training'),      items: [{ id:'schedule', icon:'ti-calendar', label:tx('nav.schedule','Schedule') }] },
  { section: tx('nav.people','People'), items: [{ id:'athletes-all', icon:'ti-run', label:tx('nav.athletes','Athletes') }, { id:'coaches', icon:'ti-user-star', label:tx('nav.coaches','Coaches') }, { id:'employees', icon:'ti-id-badge-2', label:tx('nav.employees','Staff') }, { id:'referees', icon:'ti-flag-2', label:tx('nav.referees','Referees') }] },
  { section: tx('nav.mycompetitions','My Competitions'), items: [{ id:'sports', icon:'ti-ball-football', label:tx('nav.sports','Sports') }, { id:'special-olympics', icon:'ti-medal-2', label:tx('nav.specialOlympics','Special Olympics') }, { id:'events', icon:'ti-calendar-event', label:tx('nav.events','Events') }, { id:'athlete-results', icon:'ti-medal', label:tx('nav.results','Results') }] },
  { section: tx('nav.account','Account'),         items: [{ id:'settings', icon:'ti-settings', label:tx('nav.settings','Settings') }] },
]
...
export default function App() {
  ...
  // navState carries one-time navigation intent (e.g. "open Athletes
  // pre-filtered to Status=Active" from a Dashboard KPI card). It must be
  // consumed exactly once by the page that reads it — if left sitting in
  // navState, a page remount for any unrelated reason (the global Refresh
  // button bumping refreshToken, a nav-click "reset" remount, etc.)
  // re-applies the same stale value every time, silently overriding
  // whatever the person had actually chosen since. Pages that read a
  // one-time value from navState must call this right after applying it,
  // removing just that key so nothing but a fresh, deliberate navigation
  // (a new goTo() call) can ever set it again.
  function consumeNavStateKey(key) {
    setNavState(prev => {
  ...
  // The linked role record for the CURRENT role, so its name/name_ar can
  // be shown reactively per the active UI language — profile.full_name is
  // a static string captured once at signup time (whatever language was
  // active then) and never updates when the user switches languages later.
  const myNameRecord = isAthlete ? myAthlete : isCoach ? myCoachRecord : (isEmployee || isAdmin) ? myEmployeeRecord : null
  const userName = myNameRecord
    ? (lang === 'ar' && myNameRecord.name_ar ? myNameRecord.name_ar : myNameRecord.name)
    : (profile?.full_name || (user.email?.endsWith('@qpc-system.qa') ? user.email.replace('@qpc-system.qa','') : user.email))
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden' }}>
      ...
      <div className="main">
        ...
        <div id="content">
          ...
          {page==='special-olympics' && (isAdmin || isCoach || isEmployee || isAthlete) && <SpecialOlympics key={`special-olympics-${refreshToken}`} athletes={athletes} coaches={coaches} onNav={goTo} profile={profile} />}
          {page==='employees' && (isAdmin || isCoach || isEmployee || isAthlete) && <Employees key={`employees-${refreshToken}`} employees={employees} coaches={coaches} personDocs={personDocs} onRefresh={fetchAll} onNav={goTo} initEmployeeId={navState.employeeId} navState={navState} profile={profile} />}
        </div>
      </div>
      <ToastContainer />
      </div>
